...
async def read_users(token: Annotated[str, Depends(dependencies.oauth2_scheme)], skip: int = 0, limit: int = 100, db: Session = Depends(dependencies.get_db)):
    users = crud.get_users(db, skip=skip, limit=limit)
    return users


@router.get("/{user_id}", response_model=schemas.User)
async def read_user(token: Annotated[str, Depends(dependencies.oauth2_scheme)], user_id: int, db: Session = Depends(dependencies.get_db)):
